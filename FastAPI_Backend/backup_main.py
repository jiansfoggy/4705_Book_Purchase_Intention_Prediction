...
def ensure_table(table_name=DDB_TABLE_NAME, create_if_missing=True,
    ...
    try:
        ...
        if has_table and code in ("ResourceNotFoundException", "ValidationException"):
...
def log_cache(text,pred,true_label):
    text_hash = hashlib.sha256(text.encode("utf-8")).hexdigest()
    data = {
...
def load_model_from_wandb(model_name="MultinomialNB-artifact", alias="production"):
    """
    ...
    try:
        ...
        artifact = wandb.use_model(f"{model_name}:{alias}")
...
def predict(input_data: TextInput):
    ...
    table = ensure_table(has_table=False)  # set True if you want code to auto-create table
    ...
    model = load_model_from_wandb(model_name="MultinomialNB-artifact", alias="production")
    prediction = model.predict([text])[0]
    pred = category[int(prediction)]

    log_cache(text,pred,true_label)
